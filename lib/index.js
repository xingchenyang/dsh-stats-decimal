...
import { costOf, bandForTime, isSameLocalDay, mergePricing, FALLBACK_MODEL } from "./pricing.js";
...
const ZERO_LEDGER = () => ({ cumulative: { CNY: 0, USD: 0 }, today: { CNY: 0, USD: 0 } });
...
/** Fold one usage record into the running ledger (pure). */
function foldUsage(state, buckets, model, band, currencies, table, today) {
	...
	return changed ? { cumulative, today: todayCur } : state;
}
...
function apply(ctx, config) {
	...
	const billingLedger = {
		...
		apply: (state, event) => {
			...
			return foldUsage(state, buckets, model, band, activeCurrencies, table, isSameLocalDay(event.time));
		},
		view: (state) => ({
			enabled: costEnabled,
	...
	if (true) {
		const balanceEnabled = Boolean(cfg.balance?.enabled);
		const apiBase = (cfg.balance.apiBase ?? "https://api.deepseek.com").replace(/\/+$/, "");
